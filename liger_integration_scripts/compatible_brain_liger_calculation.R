...
#load files for brain
file_activity_matrix_atac <- "/dss/dsshome1/lxc02/di82cox/code/sheetal/brain_atac_activity_matrix.rds"
activity.matrix <-readRDS(file_activity_matrix_atac)
preprocessed_rna <- readRDS("/dss/dsshome1/lxc02/di82cox/code/sheetal/brain_rna_preprocessed.rds")
raw_atac <- readRDS("/dss/dsshome1/lxc02/di82cox/code/sheetal/brain_atac.rds")
...
atac <- subset(atac, subset = nFeature_ATAC > 200 & nFeature_ATAC < 6000)
atac <- subset(atac, subset = nCount_ATAC < 6000)
atac
...
rna@meta.data$paper.cell.type <- preprocessed_rna@meta.data$paper.cell.type
#rna <- subset(rna, subset = atac.barcode != 0)
#rna <- subset(rna, subset = nFeature_RNA > 200 & nFeature_RNA < 7500)
...
saveRDS(combined,"/dss/dsshome1/lxc02/di82cox/code/sheetal/compatible_brain_combined.rds")
...
DimPlot(combined,group.by =c("orig.ident"), ncol = 1)
DimPlot(combined, group.by = c("paper.cell.type","seurat_clusters","clusters"), ncol = 3)

###########save anndata###
library(SeuratDisk)
SaveH5Seurat(combined,"/dss/dsshome1/lxc02/di82cox/code/sheetal/compatible_brain_2",overwrite=TRUE)
adata <- Convert("/dss/dsshome1/lxc02/di82cox/code/sheetal/compatible_brain_2.h5seurat","/dss/dsshome1/lxc02/di82cox/code/sheetal/compatible_brain.h5ad",assay="RNA",overwrite=TRUE)
